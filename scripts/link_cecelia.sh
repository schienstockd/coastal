...
# to cecelia's Python (in cecelia-feijoa/python/) are picked up live — no reinstall per change.
# (A plain `pip install <wheel>` copies a frozen snapshot instead — stale after any cecelia edit.)
#
# Usage:
#   scripts/link_cecelia.sh                 # default: ../cecelia/cecelia-feijoa/python
#   CECELIA_PYTHON=/path/to/cecelia-feijoa/python scripts/link_cecelia.sh
#
...
cecelia_python="${CECELIA_PYTHON:-$here/../../cecelia/cecelia-feijoa/python}"

if [[ ! -f "$cecelia_python/pyproject.toml" ]]; then
  echo "error: no cecelia package at: $cecelia_python" >&2
  echo "       (is cecelia-feijoa checked out on a branch that has python/? — it landed on main" >&2
